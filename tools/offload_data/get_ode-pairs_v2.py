...
from helios.utils.utils_base_v2 import encode_prompt, load_extra_components
...
def main():
    ...
    # Load audio VAE (DAC)
    vae = DAC.from_pretrained(args.audio_vae_path).to(device=device)
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                all_sections_ode = pipe(
                    ...
                    return_ode_stages=True, # Custom flag to indicate ODE stage generation if implemented
                )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate ODE Pairs with model for T2A")
    ...
    # For T2A we represent time/sequence natively, so height/width shouldn't be required but kept for pipeline config
    ...
    parser.add_argument(
        "--negative_prompt",
        type=str,
        default="low quality, bad quality, terrible quality",
    )
    parser.add_argument(
        "--prompt_txt_path",
        type=str,
