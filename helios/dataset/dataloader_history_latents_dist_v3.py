...
class BucketedFeatureDataset(Dataset):
    ...
    def prepare_stage1_latent(self, vae_latent, idx, base_vae_latent=None):
        ...
        # ===== Selector: 提取 GAP 帧 =====
        # GAP 区域定义（在 continue_source_latent 坐标系中）：
        #   - gap_start = history_window_size: zero padding 结束，原始视频帧 0 的位置
        #   - gap_end = start_indice: 当前 history 窗口的起始位置
        # 跳过帧 0（已有 x0_latent），所以从 gap_start+1 开始取
        # 绝对帧索引 = continue 坐标 - history_window_size（去掉 zero padding 偏移）
        gap_start = history_window_size  # continue 坐标中原始视频帧 0 的位置
        gap_end = start_indice           # continue 坐标中 history 窗口起始
        if gap_end > gap_start + 1:
            gap_latent = continue_source_latent[:, gap_start + 1 : gap_end, :, :]
            # 绝对帧索引：在原始视频中的位置（帧0=x0, 帧1=第二帧, ...）
            gap_frame_indices = torch.arange(gap_start + 1, gap_end) - history_window_size  # [1, 2, ..., N]
        else:
            gap_latent = None
            gap_frame_indices = None

        # history 帧的绝对帧索引（在原始视频中的位置）
        hist_abs_start = start_indice  # continue 坐标中 history 起始
        hist_abs_indices = torch.arange(hist_abs_start, hist_abs_start + history_window_size) - history_window_size
        # hist_abs_indices 中 < 0 的部分对应 zero padding（不是真实帧）
        # target 帧的绝对帧索引
        target_abs_start = start_indice + history_window_size
        target_abs_indices = torch.arange(target_abs_start, target_abs_start + latent_window_size) - history_window_size
