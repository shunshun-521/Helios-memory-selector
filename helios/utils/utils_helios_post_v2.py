...
from .utils_base_v2 import apply_schedule_shift
from .utils_helios_base_v2 import (
...
def _ode_regression_loss(
    ...
    # T2A: noise shape is (B, C, L)
    batch_size, num_channels_latents, latent_window_size = noise.shape
    assert ode_latents[0][0]["latents"][0].shape[0] == batch_size
...
def prepare_real_latents_for_gan(
    ...
    # NOTE: Audio latent doesn't use video vae.config properties. Skip mean/std.
    ...
    for i in range(num_sections):
        ...
        with torch.no_grad():
            decoded = vae.decode(cur_section.to(vae.device, dtype=vae.dtype))
        total_frame_latent.append(decoded)

    combined_frames = torch.cat(total_frame_latent, dim=2).to(vae.device, dtype=vae.dtype)
    num_rgb_frames = num_critic_input_frames  # Roughly scale if needed
    ...
    with torch.no_grad():
        posterior, _, _, _, _ = vae.encode(selected_frames)
        reconstructed_latent = posterior.mode()
        gan_vae_latents = reconstructed_latent
...
def sample_block_noise(scheduler, batch_size, channel, seq_len):
    ...
    block_number = batch_size * channel * seq_len

    noise = dist.sample((block_number,)) 
    noise = noise.view(batch_size, channel, seq_len)
    return noise
...
def add_noise(original_samples, noise, timestep, sigmas, timesteps):
    ...
    # 3D: (B, 1, 1)
    sigma = sigmas[timestep_id].reshape(-1, 1, 1)
...
def inference_with_trajectory_stage2(
    ...
    if is_multi_pyramid_stage_backward_simulated:
        ...
        pyramid_stage_videos = torch.zeros(
            batch_size,
            num_channels_latents,
            sum(history_sizes) // divisor, # Assuming L dims, not precise, check later
            device=accelerator.device,
            dtype=torch.float32,
        )
    ...
    for k in range(num_rollout_sections):
        ...
        for i_s in range(stage2_num_stages):
            ...
            if i_s > 0:
                ...
                num_frmaes_pyramid *= 2
                noisy_model_input = F.interpolate(
                    noisy_model_input, size=(num_frmaes_pyramid,), mode="nearest"
                )
                ...
                batch_size, channel, num_frames = noisy_model_input.shape
                noise = sample_block_noise(scheduler, batch_size, channel, num_frames)
...
def _generator_loss(
    ...
    if use_dynamic_shifting:
        temp_length = noise.shape[-1]
        if is_multi_pyramid_stage_backward_simulated:
            divisor = 2 ** (stage2_num_stages - 1 - init_pyramid_stage_flag)
            temp_length = temp_length // divisor
        
        temp_tensor = torch.randn(1, 16, temp_length) # dummy for apply_schedule_shift
    ...
    if is_smoothness_loss or is_dmd_vae_decode:
        ...
        for i in range(num_sections):
            ...
            with torch.no_grad():
                decoded = vae.decode(cur_section.to(vae.dtype))
            ...
            if is_smoothness_loss:
                with torch.no_grad():
                    posterior, _, _, _, _ = vae.encode(decoded[:, :, -1:])
                    prev_last_frame_latent = posterior.mode()
...
def _critic_loss(
    ...
    if use_dynamic_shifting:
        ...
        temp_tensor = torch.randn(1, 16, temp_length)
