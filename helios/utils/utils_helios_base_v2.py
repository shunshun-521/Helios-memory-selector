...
from .utils_base import get_config_value
...
def downsample_corrupt(model_input, downsample_min_corrupt_ratio, downsample_max_corrupt_ratio):
    ...
    # In T2A, model_input is 3D: (B, C, L)
    is_3d = model_input.ndim == 3
    if is_3d:
        B, C, L = model_input.shape
    else:
        raise ValueError(f"Expected 3D input (B, C, L), got {model_input.ndim}D")

    l0 = model_input.shape[-1]
    l1 = max(1, int(round(l0 * corrupt_ratio)))

    model_input = F.interpolate(model_input, size=l1, mode="linear", align_corners=False)
    model_input = F.interpolate(model_input, size=l0, mode="linear", align_corners=False)
...
def prepare_stage1_clean_input_from_latents(
    history_latents,  # VAE latents, (B, C_latent, L_latent)
    target_latents,
    x0_latents=None,
...
def prepare_stage2_clean_input(
    ...
    for _ in range(pyramid_stage_num - 1):
        L //= 2
        latents = torch.nn.functional.interpolate(latents, size=L, mode="linear")
        pyramid_latent_list.append(latents)
    ...
    for i_s in range(pyramid_stage_num - 1):
        L //= 2
        cur_noise = F.interpolate(cur_noise, size=L, mode="linear") * 2
    ...
    for i_s, cur_sample_ratio in zip(range(pyramid_stage_num), stage2_sample_ratios):
        clean_latent = pyramid_latent_list[i_s]  # [bs, c, L]
