...
def pad_for_1d_conv(x: torch.Tensor, kernel_size: int):
    b, c, l = x.shape
    del b, c
    pad_l = (kernel_size - (l % kernel_size)) % kernel_size
    return torch.nn.functional.pad(x, (0, pad_l), mode="replicate")


def center_down_sample_1d(x: torch.Tensor, kernel_size: int):
    return torch.nn.functional.avg_pool1d(x, kernel_size, stride=kernel_size)


def apply_rotary_emb_transposed(hidden_states: torch.Tensor, freqs_cis: torch.Tensor):
...
class Discriminator1DHead(nn.Module):
    def __init__(self, input_channel, cond_map_dim=768):
        super().__init__()

        self.head1d = nn.Sequential(
            nn.Conv1d(input_channel, cond_map_dim, 3, stride=1, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
            nn.Conv1d(cond_map_dim, cond_map_dim, 4, stride=2, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
            nn.Conv1d(cond_map_dim, cond_map_dim, 4, stride=2, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
            nn.Conv1d(cond_map_dim, cond_map_dim, 3, stride=2, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
            nn.Conv1d(cond_map_dim, cond_map_dim, 3, stride=2, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
            nn.Conv1d(cond_map_dim, cond_map_dim, 3, stride=1, padding=1),
            nn.GroupNorm(32, cond_map_dim),
            nn.SiLU(False),
...
class HeliosRotaryPosEmbed(nn.Module):
    def __init__(self, rope_dim, theta):
        ...
        self.DL = rope_dim
        self.theta = theta
        self.register_buffer("freqs_base_l", self._get_freqs_base(self.DL), persistent=False)
    ...
    def get_frequency_batched(self, freqs_base, pos):
        freqs = torch.einsum("d,bl->dbl", freqs_base, pos)
        freqs = freqs.repeat_interleave(2, dim=0)
        return freqs.cos(), freqs.sin()

    @torch.no_grad()
    def forward(self, seq_indices, device):
        B = seq_indices.shape[0]
        L = seq_indices.shape[1]

        seq_indices = seq_indices.to(device=device, dtype=torch.float32)

        # pos shape: (B, L)
        GL = seq_indices
        FCL, FSL = self.get_frequency_batched(self.freqs_base_l, GL)

        result = torch.cat([FCL, FSL], dim=0)
...
class HeliosTransformer1DModel(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for audio-like data (1D) used in the Helios model.

    Args:
        patch_size (`int`, defaults to `1`):
            1D patch dimension for audio embedding (l_patch).
    ...
    def __init__(
        ...
        self.patch_embedding = nn.Conv1d(in_channels, inner_dim, kernel_size=patch_size, stride=patch_size)
        ...
        # Compute patch_size product for projection layer properly based on tuple or int
        if isinstance(patch_size, (tuple, list)):
            patch_volume = math.prod(patch_size)
        else:
            patch_volume = patch_size
        ...
        if has_multi_term_memory_patch:
            # 1D versions of history patches, kernels roughly matching the downsampling rates of the 3D ones
            self.patch_short = nn.Conv1d(in_channels, self.inner_dim, kernel_size=1, stride=1)
            self.patch_mid = nn.Conv1d(in_channels, self.inner_dim, kernel_size=2, stride=2)
            self.patch_long = nn.Conv1d(in_channels, self.inner_dim, kernel_size=4, stride=4)
            self.initialize_weight_from_another_conv1d(self.patch_embedding)
        ...
        if is_use_gan:
            ...
            if is_use_gan_hooks:
                ...
                for hook in self.gan_hooks:
                    gan_heads.append((str(hook), Discriminator1DHead(inner_dim, gan_cond_map_dim)))
                self.gan_heads = nn.ModuleDict(gan_heads)
            if is_use_gan_final:
                self.gan_final_head = Discriminator1DHead(out_channels, gan_cond_map_dim)

        self.gradient_checkpointing = False

    @torch.no_grad()
    def initialize_weight_from_another_conv1d(self, another_layer):
        weight = another_layer.weight.detach().clone()
        bias = another_layer.bias.detach().clone()

        # The input weight from another_layer (like patch_embedding) 
        # might still be initialized as 5D by default diffusers config loading 
        # if the config contains 3D dimensions before we manually override it.
        # We enforce it to be 1D by taking the center/first element if needed.
        if weight.ndim == 5:
        # 1. 先把 H 和 W 维度的能量平均掉，得到 [B, C, T]
        # 比如 [5120, 16, 1, 2, 2] -> [5120, 16, 1]
            weight = weight.mean(dim=(3, 4))

        # Assuming in_channels is 128
        ...
        sd = {
            ...
            "patch_mid.weight": einops.repeat(weight, "b c l -> b c (l lk)", lk=2) / 2.0,
            "patch_mid.bias": bias.clone(),
            "patch_long.weight": einops.repeat(weight, "b c l -> b c (l lk)", lk=4) / 4.0,
    ...
    def process_input_hidden_states(
        ...
        if isinstance(latents, list):
            ...
            for idx, cur_hidden_states in enumerate(latents):
                ...
                B, C, L = cur_hidden_states.shape

                cur_hidden_states = cur_hidden_states.transpose(1, 2)

                if indices_hidden_states is None:
                    indices_hidden_states = torch.arange(0, L).unsqueeze(0).expand(B, -1)

                cur_indices_latents = indices_hidden_states
                cur_rope_freqs = self.rope(
                    seq_indices=cur_indices_latents, device=cur_hidden_states.device
                )

                temporal_list.append(L)
            ...
            rope_freqs = self.rope(
                seq_indices=indices_hidden_states,
        ...
        if latents_history_short is not None and indices_latents_history_short is not None:
            ...
            latents_history_short = latents_history_short.transpose(1, 2)

            rope_freqs_history_short = self.rope(
                seq_indices=indices_latents_history_short,
                device=latents_history_short.device,
            )

            # 加上这两行调试代码
            # print(f"DEBUG: rope_freqs_history_short shape = {rope_freqs_history_short.shape}")
            # print(f"DEBUG: rope_freqs shape = {rope_freqs.shape}")
        ...
        if latents_history_mid is not None and indices_latents_history_mid is not None:
            ...
            latents_history_mid = pad_for_1d_conv(latents_history_mid, 2)
            latents_history_mid = self.gradient_checkpointing_method(self.patch_mid, latents_history_mid)
            latents_history_mid = latents_history_mid.transpose(1, 2)

            rope_freqs_history_mid = self.rope(
                seq_indices=indices_latents_history_mid,
                device=latents_history_mid.device,
            )
            # Match 1D downsampling
            rope_freqs_history_mid = rope_freqs_history_mid.transpose(1, 2) # (B, L, C) -> (B, C, L)
            rope_freqs_history_mid = pad_for_1d_conv(rope_freqs_history_mid, 2)
            rope_freqs_history_mid = center_down_sample_1d(rope_freqs_history_mid, 2)
            rope_freqs_history_mid = rope_freqs_history_mid.transpose(1, 2) # (B, C, L) -> (B, L, C)
        ...
        if latents_history_long is not None and indices_latents_history_long is not None:
            ...
            latents_history_long = pad_for_1d_conv(latents_history_long, 4)
            latents_history_long = self.gradient_checkpointing_method(self.patch_long, latents_history_long)
            latents_history_long = latents_history_long.transpose(1, 2)

            rope_freqs_history_long = self.rope(
                seq_indices=indices_latents_history_long,
                device=latents_history_long.device,
            )
            rope_freqs_history_long = rope_freqs_history_long.transpose(1, 2)
            rope_freqs_history_long = pad_for_1d_conv(rope_freqs_history_long, 4)
            rope_freqs_history_long = center_down_sample_1d(rope_freqs_history_long, 4)
            rope_freqs_history_long = rope_freqs_history_long.transpose(1, 2)
    ...
    def forward(
        ...
        p_l = self.config.patch_size[0] if isinstance(self.config.patch_size, (list, tuple)) else self.config.patch_size


        (
            hidden_states,
            rotary_emb,
            post_patch_temporal_list,
            original_context_length_list,
        ...
        post_patch_temporal = sum(post_patch_temporal_list)
        ...
        if enable_navit:
            ...
            for (
                ...
                cur_post_patch_temporal,
            ) in zip(
                reversed(original_context_length_list),
                reversed(post_patch_temporal_list),
            ):
                ...
                cur_hidden_states = cur_hidden_states.permute(0, 3, 1, 2)
                cur_hidden_states = cur_hidden_states.flatten(2, 3)
            ...
            hidden_states = hidden_states.reshape(
                batch_size, post_patch_temporal, p_l, -1
            )
            hidden_states = hidden_states.permute(0, 3, 1, 2)
            output = hidden_states.flatten(2, 3)
    ...
    def from_pretrained(
        ...
        # ---- Config field translation: WanAudioModel -> HeliosTransformer1DModel ----
        if "dim" in config and "num_attention_heads" not in config:
            dim = config["dim"]
            num_heads = config.get("num_heads", dim // 128)
            config["num_attention_heads"] = num_heads
            config["attention_head_dim"] = dim // num_heads
            config["in_channels"] = config.get("in_dim", 128)
            config["out_channels"] = config.get("out_dim", 128)
            # ffn_dim, num_layers, text_dim, freq_dim already match names
            # rope_dim defaults to attention_head_dim if not set
            if "rope_dim" not in config:
                config["rope_dim"] = config["attention_head_dim"]
            print(f"[Config Translation] WanAudioModel -> HeliosTransformer1DModel: "
                  ...
                  f"ffn_dim={config.get('ffn_dim')}, "
                  f"num_layers={config.get('num_layers')}")

        def remap_state_dict_keys(state_dict):
            """Remap WanAudioModel key names to HeliosTransformer1DModel key names."""
            import re
            remapped = {}
            for key, value in state_dict.items():
                ...
                elif key == "head.modulation":
                    new_key = "norm_out.scale_shift_table"
                    # WanAudioModel: head.modulation is (dim*2,), HeliosTransformer1DModel: norm_out.scale_shift_table is (1, 2, dim)
                    if value.ndim == 1:
                        dim_size = value.shape[0] // 2
                        value = value.reshape(1, 2, dim_size)
                elif key == "head.head.weight":
                    new_key = "proj_out.weight"
                elif key == "head.head.bias":
                    new_key = "proj_out.bias"
                # time_embedding -> condition_embedder.time_embedder
                elif key == "time_embedding.0.weight":
                    new_key = "condition_embedder.time_embedder.linear_1.weight"
                elif key == "time_embedding.0.bias":
                    new_key = "condition_embedder.time_embedder.linear_1.bias"
                elif key == "time_embedding.2.weight":
                    new_key = "condition_embedder.time_embedder.linear_2.weight"
                elif key == "time_embedding.2.bias":
                    new_key = "condition_embedder.time_embedder.linear_2.bias"
                # time_projection -> condition_embedder.time_proj
                elif key == "time_projection.1.weight":
                    new_key = "condition_embedder.time_proj.weight"
                elif key == "time_projection.1.bias":
                    new_key = "condition_embedder.time_proj.bias"
                # text_embedding -> condition_embedder.text_embedder
                elif key == "text_embedding.0.weight":
                    new_key = "condition_embedder.text_embedder.linear_1.weight"
                elif key == "text_embedding.0.bias":
                    new_key = "condition_embedder.text_embedder.linear_1.bias"
                elif key == "text_embedding.2.weight":
                    new_key = "condition_embedder.text_embedder.linear_2.weight"
                elif key == "text_embedding.2.bias":
                    new_key = "condition_embedder.text_embedder.linear_2.bias"
                else:
                    # Block-level mappings: blocks.X.{old} -> blocks.X.{new}
                    ...
                    if m:
                        ...
                        if suffix.startswith("self_attn."):
                            rest = suffix[len("self_attn."):]
                            if rest == "q.weight":
                                suffix = "attn1.to_q.weight"
                            elif rest == "q.bias":
                                suffix = "attn1.to_q.bias"
                            elif rest == "k.weight":
                                suffix = "attn1.to_k.weight"
                            elif rest == "k.bias":
                                suffix = "attn1.to_k.bias"
                            elif rest == "v.weight":
                                suffix = "attn1.to_v.weight"
                            elif rest == "v.bias":
                                suffix = "attn1.to_v.bias"
                            elif rest == "o.weight":
                                suffix = "attn1.to_out.0.weight"
                            elif rest == "o.bias":
                                suffix = "attn1.to_out.0.bias"
                            elif rest == "norm_q.weight":
                                suffix = "attn1.norm_q.weight"
                            elif rest == "norm_k.weight":
                                suffix = "attn1.norm_k.weight"
                            else:
                                suffix = f"attn1.{rest}"
                        # cross_attn -> attn2
                        elif suffix.startswith("cross_attn."):
                            rest = suffix[len("cross_attn."):]
                            if rest == "q.weight":
                                suffix = "attn2.to_q.weight"
                            elif rest == "q.bias":
                                suffix = "attn2.to_q.bias"
                            elif rest == "k.weight":
                                suffix = "attn2.to_k.weight"
                            elif rest == "k.bias":
                                suffix = "attn2.to_k.bias"
                            elif rest == "v.weight":
                                suffix = "attn2.to_v.weight"
                            elif rest == "v.bias":
                                suffix = "attn2.to_v.bias"
                            elif rest == "o.weight":
                                suffix = "attn2.to_out.0.weight"
                            elif rest == "o.bias":
                                suffix = "attn2.to_out.0.bias"
                            elif rest == "norm_q.weight":
                                suffix = "attn2.norm_q.weight"
                            elif rest == "norm_k.weight":
                                suffix = "attn2.norm_k.weight"
                            else:
                                suffix = f"attn2.{rest}"
                        # ffn.0 -> ffn.net.0.proj, ffn.2 -> ffn.net.2
                        elif suffix == "ffn.0.weight":
                            suffix = "ffn.net.0.proj.weight"
                        elif suffix == "ffn.0.bias":
                            suffix = "ffn.net.0.proj.bias"
                        elif suffix == "ffn.2.weight":
                            suffix = "ffn.net.2.weight"
                        elif suffix == "ffn.2.bias":
                            suffix = "ffn.net.2.bias"
                        # modulation -> scale_shift_table (reshape 1D -> 3D)
                        elif suffix == "modulation":
                            suffix = "scale_shift_table"
                            # WanAudioModel: modulation is (dim*6,), HeliosTransformer1DModel: scale_shift_table is (1, 6, dim)
                            if value.ndim == 1:
                                dim_size = value.shape[0] // 6
                                value = value.reshape(1, 6, dim_size)
                        # norm3 -> norm2 (cross-attn norm)
                        elif suffix == "norm3.weight":
                            suffix = "norm2.weight"
                        elif suffix == "norm3.bias":
                            suffix = "norm2.bias"
...
if __name__ == "__main__":
    ...
    transformer = HeliosTransformer1DModel.from_pretrained(
        "/root/autodl-fs/Wan-AI/Wan2.1-T2V-14B-Diffusers",
    ...
    # Using real data property from new_train.json
    # Cut duration: 8.0 seconds
    # Sample rate: 48000 Hz
    # DAC VAE hop_length: 960 (from dac_vae.md)
    # So seq_len = (8.0 * 48000) / 960 = 400
    seq_len = 400

    noisy_model_input = torch.randn(batch_size, 128, seq_len).to(device, dtype=weight_dtype)
    timesteps = torch.randint(0, 1000, (batch_size,)).to(device)
    prompt_embeds = torch.randn(batch_size, max_length, 4096).to(device, dtype=weight_dtype)
    
    indices_hidden_states = torch.arange(seq_len).unsqueeze(0).expand(batch_size, -1).to(device)
    
    indices_latents_history_short = torch.arange(seq_len // 2).unsqueeze(0).expand(batch_size, -1).to(device)
    indices_latents_history_mid = torch.arange(seq_len // 4).unsqueeze(0).expand(batch_size, -1).to(device)
    indices_latents_history_long = torch.arange(seq_len // 8).unsqueeze(0).expand(batch_size, -1).to(device)
    
    latents_history_short = torch.randn(batch_size, 128, seq_len // 2).to(device, dtype=weight_dtype)
    latents_history_mid = torch.randn(batch_size, 128, seq_len // 4).to(device, dtype=weight_dtype)
    latents_history_long = torch.randn(batch_size, 128, seq_len // 8).to(device, dtype=weight_dtype)

    print("Running forward pass test for 1D Audio Transformer...")
    ...
    print(f"Success! Output shape: {model_pred.shape}")
