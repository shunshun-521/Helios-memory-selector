...
class HeliosTransformer3DModel(
    ...
    def __init__(
        ...
        if has_multi_term_memory_patch:
            ...
            # Selector: 选中帧的 patch embedding，与 patch_short 相同 stride
            self.patch_selected = nn.Conv3d(in_channels, self.inner_dim, kernel_size=(1, 2, 2), stride=(1, 2, 2))
            # Temporal Token Selection v2.1: Q=gap(4×4 spatial), K=text+target(4×4)+timestep
            selector_dim = 128
            self._selector_dim = selector_dim
            self._selector_max_len = 512
            self._selector_spatial_size = 4  # 4×4 = 16 spatial tokens
            self._selector_pool2d = nn.AdaptiveAvgPool2d((4, 4))
            # Q branch: gap frame latent → selector_dim (per spatial position)
            self.selector_q_proj = nn.Linear(in_channels, selector_dim)
            self.selector_q = nn.Linear(selector_dim, selector_dim, bias=False)
            # K branch: text + timestep → global K; target → spatial K
            self.selector_text_proj = nn.Linear(text_dim, selector_dim)  # text_dim=4096
            self.selector_target_proj = nn.Linear(in_channels, selector_dim)  # per spatial position
            self.selector_time_proj = nn.Linear(inner_dim, selector_dim)  # temb dim=inner_dim
            self.selector_k = nn.Linear(selector_dim, selector_dim, bias=False)  # global K only
            # Learnable spatial positional encoding for 16 positions
            self.selector_spatial_pe = nn.Parameter(torch.zeros(1, 1, 16, selector_dim))
            nn.init.trunc_normal_(self.selector_spatial_pe, std=0.02)
            # Sinusoidal temporal positional encoding
            pe = torch.zeros(1, self._selector_max_len, selector_dim)
            position = torch.arange(0, self._selector_max_len).unsqueeze(1).float()
            div_term = torch.exp(torch.arange(0, selector_dim, 2).float() * (-math.log(10000.0) / selector_dim))
            pe[0, :, 0::2] = torch.sin(position * div_term)
            pe[0, :, 1::2] = torch.cos(position * div_term)
            self.register_buffer('selector_pe', pe, persistent=False)
    ...
    def _temporal_token_select(self, gap_latents, gap_frame_indices, target_latents, history_latents,
                                prompt_embeds=None, temb=None,
                                k_select=4, cross_frame_alpha=0.85, gumbel_tau=1.0):
        """
        Temporal Token Selection v2.1: Q=gap(4×4 spatial), K=text+target(4×4)+timestep.

        Dual-path relevance scoring:
        - spatial_score: per-position matching between gap frames and noisy target (4×4 grid)
        - global_score: gap frame global feature vs text+timestep semantic signal
        
        gap_latents: (B, C, T_gap, H, W) - candidate frames
        target_latents: (B, C, T_tgt, H, W) - noisy target being denoised
        history_latents: (B, C, T_hist, H, W) - recent history (redundancy penalty)
        prompt_embeds: (B, S, text_dim) - text embeddings
        temb: (B, inner_dim) - timestep embedding
        Returns: selected_latents (B, C, k, H, W), selected_indices (B, k)
        """
        B, C, T_gap, H, W = gap_latents.shape
        S = 16  # 4×4 spatial tokens
        if T_gap <= k_select:
            if self.training:
                print(f"[DEBUG Selector] Early exit: T_gap={T_gap} <= k_select={k_select}, returning all")
            return gap_latents, gap_frame_indices

        # === Q branch: gap frames → 4×4 spatial pool → per-position projection ===
        x = gap_latents.float().permute(0, 2, 1, 3, 4).reshape(B * T_gap, C, H, W)
        x = self._selector_pool2d(x)                          # (B*T_gap, C, 4, 4)
        x = x.reshape(B, T_gap, C, S).permute(0, 1, 3, 2)    # (B, T_gap, 16, C)
        Q = self.selector_q(self.selector_q_proj(x))           # (B, T_gap, 16, dim)
        # Add temporal PE: (B, T_gap, 1, dim)
        gap_pos = gap_frame_indices.long().clamp(0, self._selector_max_len - 1)
        Q = Q + self.selector_pe[0, gap_pos].unsqueeze(2)
        # Add spatial PE: (1, 1, 16, dim)
        Q = Q + self.selector_spatial_pe

        # === K-spatial branch: noisy target → 4×4 pool → per-position ===
        y = target_latents.float().mean(dim=2)                 # (B, C, H, W) temporal mean
        y = self._selector_pool2d(y)                           # (B, C, 4, 4)
        k_target_feat = y.flatten(2).permute(0, 2, 1)         # (B, 16, C)
        K_spatial = self.selector_target_proj(k_target_feat)   # (B, 16, dim)
        K_spatial = K_spatial + self.selector_spatial_pe.squeeze(1)  # add spatial PE

        # === K-global branch: text + timestep (no target) ===
        if prompt_embeds is not None:
            text_feat = self.selector_text_proj(prompt_embeds.float().mean(dim=1))
        else:
            text_feat = torch.zeros(B, self._selector_dim, device=gap_latents.device)
        if temb is not None:
            time_feat = self.selector_time_proj(temb.float())
        else:
            time_feat = torch.zeros(B, self._selector_dim, device=gap_latents.device)
        K_global = self.selector_k(text_feat + time_feat)      # (B, dim)

        # === Dual-path relevance ===
        # Spatial score: (B, T_gap, 16, dim) x (B, 16, dim)^T → mean over spatial
        spatial_score = torch.einsum('bgsd,bsd->bgs', Q, K_spatial).mean(dim=-1)  # (B, T_gap)
        # Global score: Q spatial-mean → (B, T_gap, dim) x (B, dim)
        Q_global = Q.mean(dim=2)                               # (B, T_gap, dim) TODO
        global_score = torch.einsum('bgd,bd->bg', Q_global, K_global)  # (B, T_gap)
        relevance = spatial_score + global_score                # (B, T_gap)

        # === Redundancy penalty (using global Q) ===
        if history_latents is not None and history_latents.shape[2] > 0:
            T_hist = history_latents.shape[2]
            h = history_latents.float().permute(0, 2, 1, 3, 4).reshape(B * T_hist, C, H, W)
            h = self._selector_pool2d(h).reshape(B, T_hist, C, S).permute(0, 1, 3, 2)
            Q_hist = self.selector_q(self.selector_q_proj(h))  # (B, T_hist, 16, dim)
            Q_hist_global = Q_hist.mean(dim=2)                 # (B, T_hist, dim)
            sim = torch.bmm(Q_global, Q_hist_global.transpose(1, 2))  # (B, T_gap, T_hist)
            redundancy = sim.max(dim=2).values                 # (B, T_gap)
            relevance = relevance - cross_frame_alpha * redundancy

        if self.training:
            print(f"[DEBUG Selector v2.1] relevance shape={tuple(relevance.shape)} "
                  f"min={relevance.min().item():.4f} max={relevance.max().item():.4f} "
                  f"mean={relevance.mean().item():.4f}")

        # === Gumbel-Softmax iterative selection ===
        selected_weights = torch.zeros(B, k_select, T_gap, device=gap_latents.device, dtype=gap_latents.dtype)
        mask = torch.zeros(B, T_gap, device=relevance.device)

        for i in range(k_select):
            masked_relevance = relevance + mask
            if self.training:
                soft = F.gumbel_softmax(masked_relevance, tau=gumbel_tau, hard=True, dim=-1)
            else:
                soft = torch.zeros_like(masked_relevance)
                soft.scatter_(1, masked_relevance.argmax(dim=1, keepdim=True), 1.0)
            selected_weights[:, i] = soft
            hard_idx = soft.argmax(dim=1, keepdim=True)
            mask.scatter_(1, hard_idx, float('-inf'))

        selected_latents = torch.einsum('bkt,bcthw->bckhw', selected_weights, gap_latents)

        with torch.no_grad():
            hard_indices = selected_weights.argmax(dim=2)  # (B, k)
            hard_indices = hard_indices.sort(dim=1).values
            selected_frame_indices = torch.gather(gap_frame_indices, 1, hard_indices)
            self._last_selected_indices = selected_frame_indices.detach().cpu()
    ...
    def process_input_hidden_states(
        ...
        gap_latents=None,
        gap_frame_indices=None,
        prompt_embeds=None,
        temb=None,
        k_select=4,
        cross_frame_alpha=0.85,
        # ===== Debug: absolute indices =====
        hist_abs_indices=None,
        target_abs_indices=None,
        choice_idx=None,
    ):
        height_list = []
        width_list = []
        ...
        # Save raw short history for temporal token selection (before patch embedding)
        _raw_latents_history_short = latents_history_short
        ...
        # ===== Temporal Token Selection: select from gap_latents internally =====
        if gap_latents is not None and gap_frame_indices is not None:
            # target_latents = the original latents input (before patch embedding)
            # history_latents = latents_history_short (same resolution as gap)
            target_for_select = latents if not isinstance(latents, list) else latents[-1]
            hist_for_select = _raw_latents_history_short  # raw latent before patch embedding
            selected_latents, selected_frame_indices = self._temporal_token_select(
                gap_latents=gap_latents.to(hidden_states.device, dtype=hidden_states.dtype),
                gap_frame_indices=gap_frame_indices.to(hidden_states.device),
                target_latents=target_for_select.to(hidden_states.device, dtype=hidden_states.dtype),
                history_latents=hist_for_select.to(hidden_states.device, dtype=hidden_states.dtype) if hist_for_select is not None else None,
                prompt_embeds=prompt_embeds,
                temb=temb,
                k_select=k_select,
                cross_frame_alpha=cross_frame_alpha,
            )
            selected_latents = selected_latents.to(hidden_states)
            selected_latents = self.gradient_checkpointing_method(self.patch_selected, selected_latents)
            selected_latents = selected_latents.flatten(2).transpose(1, 2)

            rope_freqs_selected = self.rope(
                frame_indices=selected_frame_indices,
                height=H1,
                width=W1,
                device=selected_latents.device,
            )
            rope_freqs_selected = rope_freqs_selected.flatten(2).transpose(1, 2)

            hidden_states = torch.cat([selected_latents, hidden_states], dim=1)
            rope_freqs = torch.cat([rope_freqs_selected, rope_freqs], dim=1)

        # ===== Debug: print absolute frame indices for all components =====
        if self.training:
            _b0 = 0  # print batch 0 only
            _parts = []
            if choice_idx is not None:
                _ci = choice_idx[_b0] if hasattr(choice_idx, '__getitem__') else choice_idx
                _parts.append(f"choice_idx={_ci}")
            if hist_abs_indices is not None:
                _ha = hist_abs_indices[_b0] if hist_abs_indices.ndim > 1 else hist_abs_indices
                from itertools import accumulate
                _history_sizes = [16, 2, 1]  # long, mid, short(1x)
                _splits = list(accumulate(_history_sizes))
                _long_abs = _ha[:_splits[0]].tolist()
                _mid_abs = _ha[_splits[0]:_splits[1]].tolist()
                _short_abs = _ha[_splits[1]:].tolist()
                _parts.append(f"target_abs={target_abs_indices[_b0].tolist() if target_abs_indices is not None else '?'}")
                _parts.append(f"short_abs={_short_abs}")
                _parts.append(f"mid_abs={_mid_abs}")
                _parts.append(f"long_abs={_long_abs}")
            else:
                if indices_hidden_states is not None:
                    _parts.append(f"target={indices_hidden_states[_b0].tolist()}")
                if indices_latents_history_short is not None:
                    _parts.append(f"short={indices_latents_history_short[_b0].tolist()}")
                if indices_latents_history_mid is not None:
                    _parts.append(f"mid={indices_latents_history_mid[_b0].tolist()}")
                if indices_latents_history_long is not None:
                    _parts.append(f"long={indices_latents_history_long[_b0].tolist()}")
            if gap_frame_indices is not None:
                _gi = gap_frame_indices.to('cpu')
                _parts.append(f"gap(T={_gi.shape[1]})=[{_gi[_b0, 0].item()}..{_gi[_b0, -1].item()}]")
            if gap_latents is not None and gap_frame_indices is not None and hasattr(self, '_last_selected_indices'):
                _parts.append(f"selected={self._last_selected_indices[_b0].tolist()}")
            print(f"[DEBUG FrameMap] {' | '.join(_parts)}")
    ...
    def forward(
        ...
        # Pre-compute lightweight temb for selector (before full condition_embedder)
        _selector_temb = None
        _selector_prompt = None
        if gap_latents is not None and gap_frame_indices is not None:
            _selector_prompt = encoder_hidden_states  # raw text embeddings (B, S, text_dim=4096)
            # Compute temb from timestep for selector
            with torch.no_grad():
                _ts = timestep[0] if isinstance(timestep, list) else timestep
                _ts_proj = self.condition_embedder.timesteps_proj(_ts)
                _te_dtype = next(iter(self.condition_embedder.time_embedder.parameters())).dtype
                if _ts_proj.dtype != _te_dtype and _te_dtype != torch.int8:
                    _ts_proj = _ts_proj.to(_te_dtype)
                _selector_temb = self.condition_embedder.time_embedder(_ts_proj).type_as(encoder_hidden_states)
        ...
        (
            ...
            prompt_embeds=_selector_prompt,
            temb=_selector_temb,
            k_select=k_select,
            cross_frame_alpha=cross_frame_alpha,
            # ===== Debug: absolute indices =====
            hist_abs_indices=hist_abs_indices,
            target_abs_indices=target_abs_indices,
