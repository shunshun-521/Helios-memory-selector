...
# Temporal Token Selection is now inside the transformer (no external selector needed)
from helios.utils.utils_helios_base_v3 import (
    _flow_loss,
...
def main(args):
    if args.data_config.use_stage3_dataset:
        ...
        from helios.dataset.dataloader_history_latents_dist_v3 import (
            BucketedFeatureDataset,
            BucketedSampler,
            collate_fn,
        )
    ...
    if getattr(args.training_config, 'use_selector', False):
        trainable_modules.extend(["selector_q_proj", "selector_q", "selector_k", "selector_text_proj", "selector_target_proj", "selector_time_proj", "selector_spatial_pe", "patch_selected"])
    for name, param in transformer.named_parameters():
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                if args.data_config.use_stage3_dataset:
                    ...
                    # ===== Debug: extract absolute indices from batch =====
                    hist_abs_indices_for_transformer = batch.get("hist_abs_indices", None)
                    target_abs_indices_for_transformer = batch.get("target_abs_indices", None)
                    choice_idx_for_transformer = batch.get("choice_idx", None)
                    if hist_abs_indices_for_transformer is not None and isinstance(hist_abs_indices_for_transformer, torch.Tensor):
                        hist_abs_indices_for_transformer = hist_abs_indices_for_transformer.to(accelerator.device)
                    if target_abs_indices_for_transformer is not None and isinstance(target_abs_indices_for_transformer, torch.Tensor):
                        target_abs_indices_for_transformer = target_abs_indices_for_transformer.to(accelerator.device)

                    # ===== Temporal Token Selection: pass gap_latents to transformer =====
                    gap_latents_for_transformer = None
                    gap_frame_indices_for_transformer = None
                    use_selector = getattr(args.training_config, 'use_selector', False)
                    if use_selector:
                        gap_latents = batch.get("gap_latents", None)
                        gap_frame_indices = batch.get("gap_frame_indices", None)
                        if isinstance(gap_latents, list):
                            gap_latents = next((x for x in gap_latents if x is not None and isinstance(x, torch.Tensor)), None)
                        if isinstance(gap_frame_indices, list):
                            gap_frame_indices = next((x for x in gap_frame_indices if x is not None and isinstance(x, torch.Tensor)), None)
                        if gap_latents is not None and isinstance(gap_latents, torch.Tensor) and gap_latents.dim() == 3:
                            gap_latents = gap_latents.unsqueeze(0)
                        if gap_frame_indices is not None and isinstance(gap_frame_indices, torch.Tensor) and gap_frame_indices.dim() == 1:
                            gap_frame_indices = gap_frame_indices.unsqueeze(0)
                        # [DEBUG Selector] Log gap_latents availability and shape
                        if global_step % 100 == 0 and accelerator.is_main_process:
                            _gl_shape = tuple(gap_latents.shape) if gap_latents is not None and isinstance(gap_latents, torch.Tensor) else None
                            _gf_shape = tuple(gap_frame_indices.shape) if gap_frame_indices is not None and isinstance(gap_frame_indices, torch.Tensor) else None
                            accelerator.print(f"[DEBUG Selector] step={global_step} gap_latents={_gl_shape} gap_frame_indices={_gf_shape}")
                        selector_dropout = getattr(args.training_config, 'selector_dropout', 0.3)
                        rand_val = torch.rand(1).item()
                        if gap_latents is not None and gap_frame_indices is not None and isinstance(gap_latents, torch.Tensor) and rand_val > selector_dropout:
                            gap_latents_for_transformer = gap_latents.to(accelerator.device, dtype=weight_dtype)
                            gap_frame_indices_for_transformer = gap_frame_indices.to(accelerator.device)
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if not args.training_config.is_train_dmd and not args.training_config.is_use_ode_regression:
                    ...
                    logs = _flow_loss(
                        ...
                        gap_latents=gap_latents_for_transformer,
                        gap_frame_indices=gap_frame_indices_for_transformer,
                        # ===== Debug: absolute indices =====
                        hist_abs_indices=hist_abs_indices_for_transformer,
                        target_abs_indices=target_abs_indices_for_transformer,
                        choice_idx=choice_idx_for_transformer,
                    )
                    optimizer.step()
                    lr_scheduler.step()
