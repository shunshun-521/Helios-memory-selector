...
from helios.utils.utils_base_v2 import (
    NORM_LAYER_PREFIXES,
    compare_configs,
    encode_prompt,
    get_optimizer,
...
def main(args):
    from helios.dataset.dataloader_history_latents_dist_v2 import (
    ...
    # Use DAC for Audio VAE instead of AutoencoderKLWan
    audio_vae_path = os.path.join(args.model_config.pretrained_model_name_or_path, "audio_vae")
    vae = DAC.from_pretrained(audio_vae_path)
    ...
    if args.training_config.is_train_dmd:
        ...
        real_score_model = HeliosTransformer1DModel.from_pretrained(
    ...
    if args.training_config.use_ema:
        model_cls = HeliosTransformer1DModel
    ...
    def load_model_hook(models, input_dir):
        ...
        if not accelerator.distributed_type == DistributedType.DEEPSPEED:
            ...
            transformer_ = HeliosTransformer1DModel.from_pretrained(
                args.model_config.transformer_model_name_or_path,
                subfolder=(
    ...
    if args.data_config.use_stage1_dataset:
        dataset_kwargs = {
            ...
            "single_seq_len": args.data_config.single_seq_len if hasattr(args.data_config, "single_seq_len") else 400,
        ...
        raise NotImplementedError("Only stage1 dataset is supported for T2A for now.")
    ...
    if args.training_config.use_error_recycling:
        ...
        resolutions = set()
        for t in sampler.buckets.keys():
            base_t = t[0]
            resolutions.add((base_t,))
            if args.training_config.is_enable_stage2:
                resolutions.add((base_t // 2,))
                resolutions.add((base_t // 4,))
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                if args.data_config.use_stage3_dataset:
                    raise NotImplementedError("T2A stage 3 dataset not yet implemented")
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if not args.training_config.is_train_dmd and not args.training_config.is_use_ode_regression:
                    ...
                    _, logs = _ode_regression_loss(
                        ...
                        transformer=transformer,
                        scheduler=noise_scheduler_copy,
                        noise=torch.randn(model_input.shape, device=accelerator.device, dtype=weight_dtype),
                    ...
                    if TRAIN_GENERATOR:
                        ...
                        if args.training_config.is_use_ode_regression:
                            ...
                            _, ode_log_dict = _ode_regression_loss(
                                ...
                                scheduler=noise_scheduler_copy,
                                noise=torch.randn(
                                    model_input.shape, device=accelerator.device, dtype=weight_dtype
                                ),
                                # For Stage 1
                                is_keep_x0=True,
                                history_sizes=args.training_config.history_sizes,
                    ...
                    critic_loss, critic_log_dict = _critic_loss(
                        ...
                        noise=torch.randn(
                            model_input.shape, device=critic_accelerator.device, dtype=weight_dtype
                        ),
                        prompt_embeds=prompt_embeds,
                        # For VRAM manager
                        dmd_is_low_vram_mode=args.training_config.dmd_is_low_vram_mode,
                    ...
                    if not (
                        USE_GAN
                        and (args.training_config.aprox_r1 or args.training_config.is_gan_low_vram_mode)
                    ):
            ...
            if accelerator.sync_gradients:
                ...
                if args.training_config.is_train_dmd:
                    if accelerator.is_main_process and VISUALIZE:
                        ...
                        if vae is None:
                            vae = DAC.from_pretrained(audio_vae_path).to(accelerator.device)
                            vae.eval()
                        ...
                        for tracker in accelerator.trackers:
                            if tracker.name == "wandb":
                                ...
                                def prepare_for_saving(tensor, caption=None):
                                    tensor = tensor.detach().cpu().numpy()
                                    if tensor.ndim == 3 and tensor.shape[0] == 1:
                                        tensor = tensor[0]
                                    return wandb.Audio(tensor.T, sample_rate=vae.sample_rate, caption=caption)
                                ...
                                for log_dict, keys in log_configs:
                                    for key in keys:
                                        if key in log_dict:
                                            ...
                                            audio_logs.append(prepare_for_saving(decoded, caption=key))
                                            del decoded

                                tracker.log({phase_name: audio_logs}, step=global_step)
                    ...
                    if accelerator.is_main_process:
                        with torch.no_grad():
                            ...
                            for validation_prompt in args.validation_config.validation_prompts:
                                ...
                                all_prompts.extend([prompt] * len(audios))
                            ...
                            for tracker in accelerator.trackers:
                                ...
                                if tracker.name == "wandb":
                                    ...
                                    for i, (audio, prompt) in enumerate(zip(all_audios, all_prompts)):
                                        filename = os.path.join(
                                            args.output_dir,
                                            f"global_step{global_step}_{phase_name}_audio_{i}_{prompt[:25].replace(' ', '_')}.wav",
                                        )
                                        audio_np = audio.cpu().numpy()
                                        if audio_np.ndim == 3 and audio_np.shape[0] == 1:
                                            audio_np = audio_np[0]
                                        sf.write(filename, audio_np.T, vae.sample_rate)
                                        audio_logs.append(
                                            wandb.Audio(filename, caption=f"{i}: {prompt}")
                                        )
                    ...
                    if tracker.name == "wandb":
                        ...
                        for i, (audio, prompt) in enumerate(zip(all_audios, all_prompts)):
                            ...
                            if audio_np.ndim == 3 and audio_np.shape[0] == 1:
                                audio_np = audio_np[0]
                            sf.write(filename, audio_np.T, vae.sample_rate)
                            audio_logs.append(wandb.Audio(filename, caption=f"{i}: {prompt}"))
...
def log_validation(
    ...
    logger.info(
        f"Running validation... \n Generating {args.validation_config.num_validation_videos} audios with prompt: {pipeline_args['prompt']}."
    )
    ...
    for _ in range(args.validation_config.num_validation_videos):
        audio = pipe(**pipeline_args, generator=generator, output_type="pt").frames
        audios.append(audio)

    del pipe
    free_memory()

    return audios, pipeline_args["prompt"]
...
if __name__ == "__main__":
    ...
    if conf.data_config.single_length:
        assert conf.data_config.force_rebuild, "force_rebuild must be True when single_length is enabled"
